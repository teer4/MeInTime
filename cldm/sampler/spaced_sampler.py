...
class SpacedSampler(Sampler):
    ...
    def apply_model(
        ...
        lambda_t = torch.full((batch_size,), 0.75, device=x.device, dtype=torch.float32)
        #lambda_t = torch.ones(batch_size, device=x.device)  # 👈 固定为1.0
        if uncond is None or cfg_scale == 1.0:
            model_output = model(x, model_t, cond, image_prompt, lambda_t)
        else:
            #image_prompt_uncond = torch.zeros_like(image_prompt)
            model_cond = model(x, model_t, cond, image_prompt, lambda_t)
            model_uncond = model(x, model_t, uncond, image_prompt, lambda_t)
    ...
    def p_sample(
        ...
        # predict x_0
        model_output = self.apply_model(model, x, model_t, cond, uncond, image_prompt, cfg_scale)
