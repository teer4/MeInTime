...
class Guidance:
    ...
    def __init__(
        ...
        """
        self.scale = scale * 3000     #梯度缩放因子，影响结果与一阶段模型RM的接近程度
        self.t_start = t_start
        self.t_stop = t_stop
        self.target = None            #I_RM
        self.space = space            #损失计算空间，RGB / latent
        self.repeat = repeat
...
class MSEGuidance(Guidance):
    ...
    def _forward(
        ...
        with torch.enable_grad():
            ...
            loss = (pred_x0 - target_x0).pow(2).mean((1, 2, 3)).sum()    #.mean(1,2,3) -> 在维度C,H,W上求整体平均（1/CHW），得到shape=(B,)  .sum()把batch中每个样本的均方误差相加    
        scale = self.scale
...
class WeightedMSEGuidance(Guidance):
    ...
    def _forward(
        ...
        # inputs: [-1, 1], nchw, rgb
        with torch.no_grad():
            w = self._get_weight((target_x0 + 1) / 2)    #将图像范围[-1,1]转换为[0,1]
        with torch.enable_grad():
            pred_x0.requires_grad_(True)
            loss = ((pred_x0 - target_x0).pow(2) * w).mean((1, 2, 3)).sum()
        scale = self.scale
        g = -torch.autograd.grad(loss, pred_x0)[0] * scale     #梯度项乘缩放系数-s*grad(z_0)
        return g, loss.item()
